...
void apply(node_t *node, char ubicacion[], char nombre[],	int pid, char estado[]){
	strcpy(node -> ubicacion, ubicacion);
	strcpy(node -> nombre, nombre);
	strcpy(node -> pid, pid);
	strcpy(node -> estado, estado);
}
...
*/
/*
int list_nondup_end_insert(list_t *list, node_t *node)
{
	...
	if(tmp==NULL){
		...
		strcpy(tmp1->nombre,node->nombre);
		tmp1->next=NULL;
		list->head=tmp1;
		count++;
		return count;
